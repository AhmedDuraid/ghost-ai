...
Both fonts are loaded via `next/font/local` from local project assets and applied as CSS variables on the `<html>` element. The base `body` uses Geist Sans with `antialiased`.
